...
# Build every package and hoist its dependencies to the top-level `node_modules/` folder
${npm_bin_dir}/lerna bootstrap --hoist
${npm_bin_dir}/lerna clean --yes

# Create symlinks from top-level `node_modules/` folder to every package
# (modified to ensure that the target directory exists)
